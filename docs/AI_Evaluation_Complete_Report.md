...
| 附录 | 文件 | 说明 |
|------|------|------|
| A | `appendices/a-installation-guides.md` | 安装指南（含本项目与对话评测） |
| B | `appendices/b-conversation-agent-eval.md` | 对话 Agent 评测说明（与 Demystifying evals 一致） |

## 快速开始

1. **阅读完整报告**：下载并阅读 `AI_Evaluation_Complete_Report.md`
2. **使用目录导航**：通过目录快速跳转到相关章节
3. **参考附录**：附录 A 为安装指南（含本项目与对话评测），附录 B 为对话 Agent 评测说明
...
- Python 3.9 或更高版本（与 `pyproject.toml` 一致）
- Docker（可选，用于容器化部署或外部基准）
- Git（可选，用于获取开源项目）
- 基本的命令行操作能力
...
- **2026-03-01**：附录与文档对齐：附录 A 聚焦本项目安装与对话评测，新增附录 B 对话 Agent 评测说明；技术要求改为 Python 3.9+
